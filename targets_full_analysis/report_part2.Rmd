...
```{r out.width = '95%', echo=FALSE}
plot(tar_read(res.mfa.b),choix="group", palette=c("red", "blue"))
```
...
fviz_mfa_ind(tar_read(res.mfa.b), habillage = tar_read(groupInfo), palette = pal) # color by cancer subtype
```

Individuals analysis with all vars
```{r out.width = '95%', echo=FALSE}
pal <- c("green", "blue", "coral", "skyblue", "red")
fviz_mfa(tar_read(res.mfa.b), habillage = tar_read(groupInfo), palette = pal)# color by groups
```
